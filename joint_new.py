...
def cnn_model_fn(features, labels, mode):
    ...
    input_feature = tf.layers.batch_normalization(input_feature)
    cnn_feature = tf.reshape(features["cnn_feature"], [-1, 1024], name="features_cnn")
    ...
    logits_cnn = tf.layers.dense(inputs=dropout, units=100, name="logits")

    print("logits", logits_cnn.shape)



    logits_t = tf.layers.dense(input_feature, units=100, name='logits_t')



    p_tl = tf.layers.dense(input_feature, units=1, name="p_tl")
    print("p_tl", p_tl.shape)
    p_cnn = tf.layers.dense(cnn_feature, units=1, name="p_cnn")

    p_tl = tf.exp(p_tl) / (tf.exp(p_tl) + tf.exp(p_cnn))
...
#    train_labels = np.concatenate((np.load("train_label.npy"), np.load("eval_label.npy")), axis=0)
    ...
    train_feature = np.load("train_features.npy")[0:num]
    ...
    cnn_feature_train = np.load('cnn_feature_train.npy')
    cnn_feature_test = np.load('cnn_feature_test.npy')
    cnn_feature_train = np.asarray(cnn_feature_train, dtype=np.float32)
    cnn_feature_test = np.asarray(cnn_feature_test, dtype=np.float32)

    #create the estimator

    sense_classifier = tf.estimator.Estimator(
        model_fn=cnn_model_fn, model_dir="tmp2/cnn_model"
    )
    ...
    train_input_fn = tf.estimator.inputs.numpy_input_fn(
        ...
        x={"x": train_data, "features": train_feature, "cnn_feature": cnn_feature_train},
        y=train_labels,
        batch_size=100,
        num_epochs=None,
        shuffle=True
    ...
    eval_input_fn = tf.estimator.inputs.numpy_input_fn(
        x={"x": eval_data, "features": test_feature, "cnn_feature": cnn_feature_test},
        y=eval_labels,
        num_epochs=1,
        shuffle=False
    )
